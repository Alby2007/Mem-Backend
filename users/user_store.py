...
CREATE INDEX IF NOT EXISTS idx_tip_followups_user_status
ON tip_followups(user_id, status)
"""


def ensure_tip_followups_table(conn: sqlite3.Connection) -> None:
...
def create_tip_followup(
    ...
    """Create a new tip followup row (position monitor entry). Returns the created row."""
    now = datetime.now(timezone.utc).isoformat()
    conn = sqlite3.connect(db_path, timeout=10)
    try:
        ...
        cur = conn.execute(
            """INSERT INTO tip_followups
               (user_id, tip_id, pattern_id, ticker, direction,
               ...
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (user_id, tip_id, pattern_id, ticker.upper(), direction,
             entry_price, stop_loss, target_1, target_2, target_3,
             position_size, 'T1', 'watching',
             regime_at_entry, conviction_at_entry, now, now),
        )
        conn.commit()
        return {'id': cur.lastrowid, 'user_id': user_id, 'ticker': ticker.upper(),
...
def get_watching_followups(db_path: str) -> List[dict]:
    """Return all active (watching/partial) tip followup rows for the position monitor."""
    conn = sqlite3.connect(db_path, timeout=10)
    try:
        ensure_tip_followups_table(conn)
        rows = conn.execute(
            """SELECT id, user_id, tip_id, pattern_id, ticker, direction,
                      entry_price, stop_loss, target_1, target_2, target_3,
                      position_size, tracking_target, status,
                      alert_level, last_alert_at, regime_at_entry, conviction_at_entry,
...
def update_followup_status(
    ...
    """Update a tip_followup row status, tracking target, or stop."""
    now = datetime.now(timezone.utc).isoformat()
    ...
    if alert_level is not None:
        ...
        updates.append('last_alert_at = ?'); params.append(now)
    params.append(followup_id)
    conn = sqlite3.connect(db_path, timeout=10)
    try:
        ensure_tip_followups_table(conn)
        conn.execute(
            f"UPDATE tip_followups SET {', '.join(updates)} WHERE id = ?", params
        )
        conn.commit()
    finally:
        conn.close()
...
def get_user_followups(db_path: str, user_id: str) -> List[dict]:
    """Return all tip followup rows for a user (all statuses)."""
    conn = sqlite3.connect(db_path, timeout=10)
    try:
        ensure_tip_followups_table(conn)
        rows = conn.execute(
            """SELECT id, user_id, tip_id, pattern_id, ticker, direction,
                      ...
                      position_size, tracking_target, status,
                      regime_at_entry, conviction_at_entry, created_at, updated_at
               FROM tip_followups WHERE user_id = ?
               ORDER BY created_at DESC""",
            (user_id,),
        ).fetchall()
        cols = ['id', 'user_id', 'tip_id', 'pattern_id', 'ticker', 'direction',
