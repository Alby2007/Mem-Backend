...
_SYSTEM_PORTFOLIO_RULE = (
    ...
    "(1) state the current price from last_price and what the price_regime means in plain English "
    "(near_52w_low = trading near lowest price of past year; near_52w_high = near highest; "
    "mid_range = somewhere in between); "
    "(2) if signal_direction is present, state the direction and — if upside_pct is present — "
    "the analyst upside implied; "
    "(3) if conviction_tier is present, state what it means for reliability; "
    "(4) if return_1m or return_1y are present, state the performance figures; "
    "(5) end with one sentence summarising the overall KB view on that holding. "
