...
_WAR_THRESHOLD   = 50   # >=50 articles → active_war
...
def _gdelt_article_count(query: str, timespan: str = '7d') -> Optional[int]:
    """Count articles matching query using GDELT artlist. Returns count or None."""
    params = {
        'query':      query,
        'mode':       'artlist',
        'format':     'json',
...
class UCDPAdapter(BaseIngestAdapter):
    ...
    def fetch(self) -> List[RawAtom]:
        ...
        for iso3, name, query in _CONFLICT_COUNTRIES:
            time.sleep(3)  # rate limit: GDELT free tier ~5 req/s
